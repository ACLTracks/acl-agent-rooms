...
class AgentStateProjector {
	...
	public function project_event( array $event ): bool {
		...
		if ( $current && $id <= (int) ( $current['last_event_id'] ?? 0 ) ) {
			return false;
		}$expires = 'error' === $map[ $type ] ? gmdate( 'Y-m-d H:i:s', time() + max( 60, min( 1800, (int) apply_filters( 'acl_ar_agent_error_ttl', 300 ) ) ) ) : null;
		return $this->presence->upsert( $room, 'agent', $agent, $map[ $type ], (string) ( $event['created_at'] ?? null ), $id, $expires );}
}
